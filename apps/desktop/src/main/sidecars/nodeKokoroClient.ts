...
export class NodeKokoroClient implements AudioRuntimeClient {
  ...
  async batchTts(
    ...
    await this.synthesizeBatch(batch, outputDir, onProgress);
    return { wavPaths };
  }
  ...
  private async synthesizeBatch(
    ...
    try {
      await runCommand(
        ...
        {
          timeoutMs,
          timeoutHint: `Node Kokoro batch synthesis timed out after ${Math.round(timeoutMs / 1000)}s. Try rendering with fewer chapters, or switch to Python expressive backend for this project.`,
